...
function waitUntilGroupMembersMoved(groupOrMembers, destinationWindowId) {
  ...
  const promisedMoved = new Promise((resolve, _reject) => {
    onTabMoved = (tabId, _moveInfo) => {
      ...
      if (toBeMovedTabs.size == 0) {
        log('waitUntilGroupMembersMoved: all members have been moved');
        resolve();
      }
    };
...
async function moveGroupBefore(group, insertBefore) {
  const members = group.$TST.memberTabs;
  const firstMemberTab = group.$TST.firstMemberTab;
...
async function moveGroupAfter(group, insertAfter) {
  const members = group.$TST.memberTabs;
  const firstMemberTab = group.$TST.firstMemberTab;
  await browser.tabGroups.move(group.id, {
...
function waitUntilGrouped(tabs, { groupId, windowId } = {}) {
  ...
  let onUpdated = null;
  const { promisedMoved, finish: finishMoved } = waitUntilGroupMembersMoved(tabs, win.id)
